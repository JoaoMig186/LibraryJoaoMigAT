...
namespace LibAPI.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("Register")]
        public async Task<IActionResult> Register([FromBody] UserDatas userDatas)
        {
            if (!ModelState.IsValid || userDatas == null)
            {
                return new BadRequestObjectResult(new { Message = "User Registration Failed" });
            }

            var identityUser = new IdentityUser() { UserName = userDatas.UserName, Email = userDatas.Email };
            var result = await userManager.CreateAsync(identityUser, userDatas.Password);
            if (!result.Succeeded)
            {
